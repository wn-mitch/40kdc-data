...
- **Variant legality uses exact assignment, not enumeration membership.** For a unit with variant-bearing model rows, the submitted counts must admit a legal per-model assignment in a valid tier. Missing required equipment, incompatible combinations, and exceeded selection budgets fail even if every individual weapon belongs to the unit. The `loadout_candidates` output limit never decides legality.
- **Candidate witnesses retain variant identity.** Variant-bearing units enumerate compatible option states through the same row candidates and budget constraints used by exact assignment. Variant-free units retain their default-per-allocation enumeration behavior. Results retain ordinal ordering, deduplication, and the explicit truncation sentinel; per-item bounds are conservative envelopes rather than proof that choices can coexist.
