...
| `linked-api/cases.json` | Each Dataset query (`find_*`, `abilities_of`, `weapons_of`, `wargear_options_of`, `base_loadout`, `maximal_loadout`, `phases_of`, `faction_of`, `abilities_of_faction`, `weapons_of_faction`) returns the expected result | per-case comparison mode: `scalar`, `ordered`, or `set` (sorted before compare) |
...
| `terrain-resolver/cases.json` | `resolveLayout(layout, templates)` resolves template-anchored, centroid-positioned pieces to absolute board-space geometry | per-value `±5e-4` on vertices/wall points; exact on identity, type, floor, roof/category, and wall thickness; piece emission order load-bearing |
...
- **Every fixture carries all eight export goldens** (the six round-trip-capable formats plus the two export-only ATC 2026 variants), including legacy ListForge fixtures whose canonical input is `input.json` (ListForge) or `input.gw.txt` (GW app). What legacy fixtures do *not* carry is the derived round-trip text input (`input.newrecruit-wtc-compact.txt`, etc.) — those are skipped because lossy multi-force decoration (provisional leader-attachment inference, multi-force warning emission) would cause the round-trip to fail structurally rather than surface a parser bug.
- **ATC 2026 (`atc-2026-compact` / `atc-2026-full`) is export-only.** Both reuse the WTC compact/full bodies byte-for-byte and only swap the summary header for the ATC list-submission block (player/team placeholders, picked Force Disposition, every enhancement-bearing model, and spelled-out leader/support attachments). There is no ATC import adapter, so these formats emit an `expected.atc-2026-*.txt` golden but **never** a derived `input.atc-2026-*.txt` round-trip seed. `PLAYER NAME` / `TEAM NAME` are static `—` placeholders (the Roster models neither); `DISPOSITION` is `titleCaseId(force_disposition)` or `—`.
...
- **Model scope (core rule 19.04) is part of the contract.** An effect whose `target` is `self` or `bearer` names a single *model*, so when the buff `source` is `{kind: "ability", abilityKind: "attached"}` — the ability was pooled in from another member of a combined unit — it contributes **no** buff and emits exactly one diagnostic: `model-scoped effect from an attached model: applies to that model only (core rule 19.04)`. The gate is keyed on the source kind (not on the DSL condition) and fires under **both** perspectives; the same effect read as the chosen unit's own ability (`abilityKind: "unit"`) still applies. Both directions are pinned (`shadowfield`/`psychic-gifts` for the drop, `mental-fortress-psychic`/`breakin-heads` for the unit-scoped counterpart) — the counterpart cases exist to catch the gate over-reaching onto `target: "unit"` rules. `abilityKind: "support"` is deliberately *not* gated.
...
- `base_loadout(unit, modelCount)` encodes the no-swaps loadout (every model in its out-of-the-box configuration) as sorted `"id:count"` strings, compared as a `set`. It pins the legal default a freshly-added unit ships with: when the composition records per-model `default_weapon_ids` those are authoritative (Σ over allocated model-types), else the derived base set. This is the load-bearing contract — `maximal_loadout` below is advisory by contrast. See `baseLoadout` in `src/data/loadout.ts` / `base_loadout` in `loadout.rs`.
- `maximal_loadout(unit, modelCount)` encodes the resulting per-weapon/wargear counts as sorted `"id:count"` strings and compares as a `set` (the id→count map has no inherent order). It pins the take-every-swap loadout maths (`optionCap` → base-minus-swaps, choices take their first declared branch); see `src/data/loadout.ts` / `loadout.rs`. Because multiple options can compete for one model's slot, the take-every-swap result is **advisory** for cap-contended units — pin `base_loadout` (the default a unit ships with), not `maximal_loadout`, for such units.
- **Whole-model variants and item options are distinct.** A composition variant supplies one model's complete initial equipment multiset; compatible `wargear_options` transform it. Repeated IDs preserve multiplicity. Variant limits count selected alternatives, while `wargear_budgets` count final equipment. A transformation into another declared variant must satisfy that destination variant's limits.
- **Variant legality uses exact assignment, not enumeration membership.** For a unit with variant-bearing model rows, the submitted counts must admit a legal per-model assignment in a valid tier. Missing required equipment, incompatible combinations, and exceeded selection budgets fail even if every individual weapon belongs to the unit. The `loadout_candidates` output limit never decides legality; exact legality is independent of candidate traversal and limit and remains exhaustive.
- **Candidate witnesses retain variant identity and deterministic provenance.** Applicable composition tiers are visited in declaration order; a tier applies when its summed model-row range contains `modelCount` (or the top-level model rows are used when there are no tiers). Each tier's allocations use pure descending recursive enumeration in model-row declaration order. Within rows, declared variants precede their compatible option states in canonical provenance order. Variant-free witnesses retain their nonzero model rows in declaration order as `<name>×<count>` groups separated by `;`; variant selections retain variant identity, and witness groups retain first-seen traversal order. Counts are ascending `id:count` pairs separated by `,`. Final encodings are globally deduplicated by first occurrence and emitted in that traversal order, not ordinal/code-point sorted. Traversal stops after distinct `limit + 1` legal output, returning the first `limit` plus `…truncated`; the default limit is 256, and `limit=0` returns the marker iff any legal candidate exists. The limit does not make proof of no solution universally bounded: rejected, duplicate, and no-solution branches may still require exhaustive search. Per-item bounds are conservative envelopes rather than proof that choices can coexist.
...
- **Composition / parenting.** A feature with `parent_area_id` (and an area template's embedded composed `features`) is placed in the parent area's centroid-local frame, then carried through the area's placement: `board = T_area ∘ R_area ∘ M_area ( featurePos + R_feat · M_feat · (w − C_feat) )`. Wall points follow the same transform. A composed feature and the same feature parented explicitly resolve identically.
- **Resolved feature metadata.** Feature templates propagate `has_roof`, `terrain_category`, and transformed `walls` (including optional thickness). A composed feature ID is namespaced as `<parent-piece-id>--<feature-id>`; an omitted feature ID falls back to `<parent-piece-id>--feature-<1-based-index>`.
...
- **Comparison:** vertices and wall points are rounded to 4 dp (JS `Math.round` semantics, `floor(x·1e4 + 0.5)/1e4`, matched in Rust) and compared per-value with the `5e-4` float tolerance; every non-coordinate field is compared exactly.
