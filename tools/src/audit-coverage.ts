 ...
 * per-faction *repair* table — the work still outstanding, not total coverage.
 * Sorted by authoring need (structural stubs first) so a contributor sees at a
 * glance which factions need the most DSL work. The full total+repair breakdown
 * and unsupported-effect histogram stay in `data/_audit/summary.md`; the
 * enumerated stub worklist (ability ids) is in `data/_audit/worklist.md`.
 */
function coverageDigest(r: CoverageReport): string {
  const rows = [...r.factions].sort(
    (a, b) =>
      b.stubStructural - a.stubStructural ||
      b.stub - a.stub ||
      b.gwTextLeak - a.gwTextLeak ||
      b.defensiveSkipped - a.defensiveSkipped ||
  ...
  for (const f of rows) {
    lines.push(
      `| ${f.faction} | ${f.stubStructural} | ${f.stub} | ${f.gwTextLeak} | ${f.defensiveSkipped} |`,
    );
  }
  const t = r.totals;
  lines.push(
    `| **TOTAL** | **${t.stubStructural}** | **${t.stub}** | **${t.gwTextLeak}** | **${t.defensiveSkipped}** |`,
    "",
    "`stub*` = empty-modifier placeholder nodes (highest need first) · `notes-stub` = flagged in " +
      "`community_notes` · `gw-leak` = suspected verbatim GW text · `def-skipped` = defensive abilities " +
      "skipped by the buff walk. Enumerated stub ids: [`data/_audit/worklist.md`](data/_audit/worklist.md). " +
      "Full total+repair breakdown: [`data/_audit/summary.md`](data/_audit/summary.md).",
    COVERAGE_END,
  );
  return lines.join("\n");
}
...
/**
 * The enumerated stub worklist (`data/_audit/worklist.md`): one row per
 * structural stub, naming the ability and the gap that needs authoring. The
 * actionable counterpart to the per-faction `stub*` counts.
 */
function worklistReport(r: CoverageReport): string {
  const stubs = r.worklist
    .filter((w) => w.stub)
    .sort((a, b) => a.faction.localeCompare(b.faction) || a.ability_id.localeCompare(b.ability_id));
  const lines: string[] = [
    ...
    "",
    `Total: ${stubs.length} stub abilities.`,
    "",
    "| faction | ability | shape | needs |",
    "|---|---|---|---|",
  ];
  for (const w of stubs) {
    lines.push(`| ${w.faction} | ${w.ability_id} | ${w.shape ?? "—"} | ${w.gap ?? "empty modifier"} |`);
  }
  lines.push("");
  return lines.join("\n");
}
...
export function auditCoverageCommand(opts: AuditCoverageOptions = {}): void {
  ...
  if (opts.write) {
    ...
    // diffable on its own and consumable without the histograms. Emit both the
    // machine-readable JSON and a human-readable stub table.
    writeFileSync(resolve(AUDIT_DIR, "worklist.json"), JSON.stringify(report.worklist, null, 2) + "\n");
    writeFileSync(resolve(AUDIT_DIR, "worklist.md"), worklistReport(report));
